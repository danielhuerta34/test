 ...
 *
 * Copyright (c) 2021 Pritesh Gupta. All Rights Reserved.
...
Class PDOCrud {
    ...
    public $maxSize = 10000000000; // Max size of file allowed for file upload
    ...
    private function initializeBtnActions() {
        if ($this->settings["actionbtn"]) {
            if ($this->settings["viewbtn"]) {
                $this->enqueueBtnActions("view", "javascript:;", "view", "<i class=\"fa fa-search\"></i>", "", array("title" => $this->langData["view"]), "btn-info");
            }
...
//        unset($this->btnActions["edit"]);
    ...
    private function formatTableData($result) {
        ...
        if (is_array($this->bulkCrudUpdateCol) && count($this->bulkCrudUpdateCol)) {
            ...
            foreach ($this->bulkCrudUpdateCol as $col => $param) {
                ...
                $data[$col] = array("field" => $this->getHTMLElement($col . "[]", $param["fieldType"], $param["attr"],  array("{val}"),  $param["fieldData"], array("input-bulk-crud-update". ' ' .$col)));
    ...
    public function getInputField($fieldName, $attr = array(), $data = array(), $type = "text", $fieldClass = array(), $fieldId = "") {
        ...
        $field = "<input type=\"$type\" class=\"form-control pdocrud-form-control pdocrud-$type $class\" id=\"$id\" name=\"$fieldName\" autocomplete=\"off\" ";
    ...
    public function getSubmitField($fieldName, $attr = array(), $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<input data-form-id=\"$formId\" type=\"submit\" class=\"btn btn-success pdocrud-form-control pdocrud-submit $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        if (is_array($attr) && count($attr)) {
            foreach ($attr as $c => $v) {
                $field .= " $c=\"$v\" ";
            }
    ...
    public function getButtonField($fieldName, $attr, $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<button data-form-id=\"$formId\" type=\"button\" class=\"btn btn-danger pdocrud-form-control pdocrud-button $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        $buttonText = "";
    ...
    public function getSearchBox($columns, $data = array()) {
        ...
        $search .= "<a href=\"javascript:;\" id=\"pdocrud_search_btn\" name=\"pdocrud_search_btn\" class=\"pdocrud-form-control pdocrud-actions btn-sm btn btn-primary $class\" data-action=\"search\">";
     ...
     */
    function fileUpload($fileName, $fileUploadPath = "", $maxSize = 10000000000000, $allowedFileTypes = array()) {
